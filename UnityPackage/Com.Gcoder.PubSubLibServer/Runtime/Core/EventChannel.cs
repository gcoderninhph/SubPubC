...
internal sealed class EventChannel : IDisposable
{
    ...
    internal ISubscrible AddOnUnitEnterBatch(Action<(List<long>, IUnit)> cb)
    {
        ...
        try { _onUnitEnterBatch.Add(cb); }
        finally { _subLock.ExitWriteLock(); }
        return new Subscrible(() =>
        {
            _subLock.EnterWriteLock();
            try { _onUnitEnterBatch.Remove(cb); }
            finally { _subLock.ExitWriteLock(); }
        });
    }
    ...
    {
        _subLock.EnterWriteLock();
        try { _onUnitLeaveBatch.Add(cb); }
        finally { _subLock.ExitWriteLock(); }
        return new Subscrible(() =>
        {
            _subLock.EnterWriteLock();
            try { _onUnitLeaveBatch.Remove(cb); }
            finally { _subLock.ExitWriteLock(); }
        });
    }
    ...
    {
        _subLock.EnterWriteLock();
        try { _onUnitEnterSync.Add(cb); }
        finally { _subLock.ExitWriteLock(); }
        return new Subscrible(() =>
        {
            _subLock.EnterWriteLock();
            try { _onUnitEnterSync.Remove(cb); }
            finally { _subLock.ExitWriteLock(); }
        });
    }
    ...
    {
        _subLock.EnterWriteLock();
        try { _onUnitLeaveSync.Add(cb); }
        finally { _subLock.ExitWriteLock(); }
        return new Subscrible(() =>
        {
            _subLock.EnterWriteLock();
            try { _onUnitLeaveSync.Remove(cb); }
            finally { _subLock.ExitWriteLock(); }
        });
    }
    ...
    {
        _subLock.EnterWriteLock();
        try { _onUnitEvent.Add(cb); }
        finally { _subLock.ExitWriteLock(); }
        return new Subscrible(() =>
        {
            _subLock.EnterWriteLock();
            try { _onUnitEvent.Remove(cb); }
            finally { _subLock.ExitWriteLock(); }
        });
    }
    ...
    internal List<Action<(List<long>, IUnit)>> SnapshotOnUnitEnterBatch()
    {
        ...
        try { return new List<Action<(List<long>, IUnit)>>(_onUnitEnterBatch); }
        finally { _subLock.ExitReadLock(); }
    }

    internal List<Action<(List<long>, IUnit)>> SnapshotOnUnitLeaveBatch()
    {
        _subLock.EnterReadLock();
        try { return new List<Action<(List<long>, IUnit)>>(_onUnitLeaveBatch); }
        finally { _subLock.ExitReadLock(); }
    }
    ...
    {
        _subLock.EnterReadLock();
        try { return new List<Action<(long, List<IUnit>)>>(_onUnitEnterSync); }
        finally { _subLock.ExitReadLock(); }
    }
    ...
    {
        _subLock.EnterReadLock();
        try { return new List<Action<(long, List<UnitKey>)>>(_onUnitLeaveSync); }
        finally { _subLock.ExitReadLock(); }
    }
    ...
    {
        _subLock.EnterReadLock();
        try { return new List<Action<(List<long>, IUnit, string, object, bool)>>(_onUnitEvent); }
        finally { _subLock.ExitReadLock(); }
    }

...
namespace PubSubLib
{
    internal sealed class PubSub : IPubSub, IPubSubInternal
    {
        ...
        private PubSub(PubSubConfig config)
        {
            ...
            _watcherExpirations = ISortedDictionary<long, long>.Create();
        ...
        public void CreateUnit<T>(long id, string type, Vector2 position, T target, Action<IUnit> onCreated,
        ...
        public Task<IUnit> CreateUnitAsync<T>(long id, string type, Vector2 position, T target, byte[]? data = null)
        ...
        public void WatcherPingUnits(long watcherId, Dictionary<string, Dictionary<long, int>> typeVersions)
        {
            _channel.Enqueue(() =>
            {
                ...
                {
                    var unitVersions = typeVersions.TryGetValue(type, out var dict)
                        ? dict
        ...
        public ISubscrible OnUnitEvent(
        ...
        private void FireBatchEnter(long[] watcherIds, IUnit unit)
        {
            ...
            try
            {
                foreach (var cb in cbs)
                {
                    ...
                    {
                        PubSubLog.Error(ex, "OnUnitEnterBatch");
                    }
                }
            }
            finally
            {
                ListPool<long>.Return(list);
            }
        }
        ...
        private void FireBatchLeave(long[] watcherIds, IUnit unit)
        {
            ...
            try
            {
                foreach (var cb in cbs)
                {
                    ...
                    {
                        PubSubLog.Error(ex, "OnUnitLeaveBatch");
                    }
                }
            }
            finally
        ...
        private void FireSyncEnter(long watcherId, List<IUnit> units)
        {
            ...
            foreach (var cb in cbs)
            {
                try
                {
                    cb.Invoke((watcherId, units));
                }
                catch (Exception ex)
                {
                    PubSubLog.Error(ex, "OnUnitEnterSync");
                }
            }
        }
        ...
        private void FireSyncLeave(long watcherId, UnitKey[] unitKeys)
        {
            ...
            try
            {
                foreach (var cb in cbs)
                {
                    try
                    {
                        cb.Invoke((watcherId, list));
                    }
                    catch (Exception ex)
                    {
                        PubSubLog.Error(ex, "OnUnitLeaveSync");
                    }
                }
            }
            finally
            {
                ListPool<UnitKey>.Return(list);
            }
        }
        ...
        private void FireUnitEvent(long[] watcherIds, IUnit unit, string eventName, object? data, bool reliable)
        {
            ...
            try
            {
                foreach (var cb in cbs)
                {
                    try
                    {
                        cb.Invoke((list, unit, eventName, data!, reliable));
                    }
                    catch (Exception ex)
                    {
                        PubSubLog.Error(ex, "OnUnitEvent");
                    }
                }
            }
            finally

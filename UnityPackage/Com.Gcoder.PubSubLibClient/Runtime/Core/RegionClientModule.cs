...
namespace PubSubLib.Client
{
    internal sealed class RegionClientModule : IRegionClientModule
    {
        ...
        private void OnPubSubEvent(PubSubEvent evt)
        {
            ...
            {
                PubSubLog.Error(ex, "RegionClientModule.OnPubSubEvent failed");
            }
        }
        ...
        private void HandleUnitEvent(UnitEventMsg msg)
        {
            ...
            switch (msg.EventName)
            {
                case "commit":
                    ...
                    {
                        PubSubLog.Error(ex, "HandleUnitEvent commit failed");
                    }
                ...
                case "message":
                    ...
                    {
                        PubSubLog.Error(ex, "HandleUnitEvent message failed");
                    }
                ...
                w =>
                {
                    try
                    {
                        ...
                        UnityEngine.Debug.LogError(
                            new Exception($"{w.GetType().FullName} failed, require {typeof(T).FullName}"));
                        return null;
                    }
                    catch (Exception ex)
                    {
